# -------------------------------------------------
# S3 Bucket for static site
# -------------------------------------------------
resource "aws_s3_bucket" "portfolio" {
  bucket_prefix = "portfolio-"  # AWS will append random suffix
}

resource "aws_s3_bucket_website_configuration" "portfolio" {
  bucket = aws_s3_bucket.portfolio.id
  index_document { suffix = "index.html" }
  error_document { key = "index.html" }
}

resource "aws_s3_bucket_public_access_block" "portfolio" {
  bucket = aws_s3_bucket.portfolio.id
  block_public_acls       = false
  block_public_policy     = false
  ignore_public_acls      = false
  restrict_public_buckets = false
}

resource "aws_s3_bucket_policy" "portfolio" {
  bucket = aws_s3_bucket.portfolio.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      Principal = "*"
      Action   = "s3:GetObject"
      Resource = "${aws_s3_bucket.portfolio.arn}/*"
    }]
  })
  depends_on = [aws_s3_bucket_public_access_block.portfolio]
}

# -------------------------------------------------
# CloudFront Distribution (with auto-generated domain)
# -------------------------------------------------
resource "aws_cloudfront_distribution" "portfolio" {
  origin {
    domain_name = aws_s3_bucket_website_configuration.portfolio.website_endpoint
    origin_id   = "S3-${aws_s3_bucket.portfolio.id}"
    custom_origin_config {
      http_port              = 80
      https_port             = 443
      origin_protocol_policy = "http-only"
      origin_ssl_protocols   = ["TLSv1.2"]
    }
  }

  enabled             = true
  is_ipv6_enabled     = true
  default_root_object = "index.html"

  # No aliases needed → uses auto-generated CloudFront URL
  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "S3-${aws_s3_bucket.portfolio.id}"
    compress         = true

    forwarded_values {
      query_string = false
      cookies { forward = "none" }
    }

    viewer_protocol_policy = "redirect-to-https"
    min_ttl                = 0
    default_ttl            = 3600
    max_ttl                = 86400
  }

  price_class = "PriceClass_100"

  # Use default CloudFront certificate (no ACM needed for non-custom domain)
  viewer_certificate {
    cloudfront_default_certificate = true
  }

  restrictions {
    geo_restriction { restriction_type = "none" }
  }

  depends_on = [aws_s3_bucket_policy.portfolio]
}

# -------------------------------------------------
# Output the live URL
# -------------------------------------------------
# Outputs
output "portfolio_url" {
  value = "https://${aws_cloudfront_distribution.portfolio.domain_name}"
}

output "s3_bucket_name" {
  value = aws_s3_bucket.portfolio.bucket
}

output "cloudfront_distribution_id" {
  value = aws_cloudfront_distribution.portfolio.id
}